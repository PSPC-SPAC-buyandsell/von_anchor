...
This section outlines utilities available within the package. Utility functions reside in files von_anchor/util.py and von_anchor/codec.py. 

Function ppjson()
###################################

File ``von_anchor/util.py`` exposes function ``ppjson()``. This utility takes a JSON serialized or serializable structure and returns a pretty-print. If the structure is not compatible with JSON, it returns a python pretty-print instead. An optional parameter allows a maximum length, at which the operation truncates the output (excluding three characters for a terminating ellipsis).

Identifier Wranglers
###################################

File ``von_anchor/util.py`` exposes several functions to work with indy-sdk identifiers (plus ``SchemaKey`` of section ``3.2.1.4zz``).
...
Table 2: Identifiers Forms within indy-sdk
...
The ``ok_did()``, ``ok_schema_id()``, ``ok_cred_def_id()``, and ``ok_rev_reg_id()`` functions return whether, on composition alone, their respective input strings represent acceptable DIDs, schema identifiers, credential definition identifiers, and revocation registry identifiers.
...
Utility ``proof_req_infos2briefs()`` takes a proof request and a list of cred-infos. Its operation matches the cred-infos against the proof request and computing any corresponding non-revocation intervals to build and return a list of cred-briefs (recall that the holder-prover anchor's proof creation method accepts either a credentials structure or a list of cred-briefs as per section ``3.2.1.11.6.5zz``). The input proof request must have credential definition identifier restrictions on all requested attribute specifications.

Utility ``proof_req_briefs2req_creds()`` takes a proof request and a list of cred-briefs and builds a requested-credentials structure for proof creation. The input proof request must have credential definition identifier restrictions on all requested attribute specifications.
...
Utility ``creds_display()`` takes an indy-sdk credentials structure, an optional filter, and an optional inclusivity toggle. The filter is a python dict mapping schema identifiers to their own python dicts, each mapping an attribute name of the schema to a value. Should the credentials structure include credentials on more than one schema, the inclusivity toggle informs the operation whether to include those that the filter does not cite (default, True) or not (False). The output is a python dict mapping credential identifiers to human-readable credential synopses; e.g.,
...
for display and triage. This operation can be useful for an actuator to work with a human operator to choose credentials of interest (dict values), isolate their credential identifiers (dict keys), and feed them back to prune_creds_json(), for example

Utility ``revoc_info()`` takes an indy-sdk credentials structure and an optional filter mapping attribute names to values. It returns a python dict mapping revocation data pairs (revocation registry identifier, credential revocation identifier within the revocation registry) to the (stringified representations of) attributes of credentials matching the input filter; e.g., 
...
for an actuator to build proof of one credential into another that stems from it (recall that an indy-sdk proof can have at most one credential per credential definition). Mapping attributes from the credential definitions in the proof to relying credential definitions must be an exercise for the actuator (or possibly the VON-X layer), but note that indy-sdk canonicalizes attribute names in proofs (also, credential offers) – the canon() utility of section ``3.2.3.4zz`` may be of interest in navigating this detail.

Codec Utilities
###################################

File ``von_anchor/codec.py`` houses utilities for encoding and decoding indy-sdk attributes. Recall that indy-sdk operates elliptic curve cryptography on (immense) numeric input, and hence its callers must (invertibly) map all attribute values to non-negative integers for processing. Each attribute in indy-sdk structures carries a dict mapping raw and encoded keys to their respective values.
...
The ``encode()`` and ``decode()`` functions convert JSON-encodeable attribute values to a (numeric string) form that the indy-sdk can use in issuing credentials, creating proofs, and in verifying proofs.
...
Solution: binhex + maxint32
****************************************************

The ``encode()`` function leaves (signed) 32-bit integers alone, encoding them to their string representations. It reserves 231, 231+1, and 231+2 as sentinels for ``None``, ``False``, and ``True`` respectively, prepending the marker digit 2 to booleans as per the list below. It transforms all other input via UTF-8, binhex, and adding 231 before outputting the corresponding string representation and prepending a digit marking the original type:

- 1 for strings (except the string with a single null character – which would otherwise decode to the empty string)
- 2 for booleans
- 3 for integers
- 4 for floating points
- 9 for any other (JSON-encodeable) type, including the string on a single null character.

The ``decode()`` function reverses the process and coerces the output into the type that its marker digit identifies – necessarily, any non-trivial encoding must result in a value that cannot correspond to a 32-bit integer.